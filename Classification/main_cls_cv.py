...
def main(cfg: DictConfig) -> None:
    ...
    with mlflow.start_run():
        ...
        for stage in range(cfg.opt.num_nets):
            ...
            stage_mdlloss = []
            for epoch in range(cfg.opt.epochs_per_stage):
            ...
            # f1_score
            val_f1_score = F1_measure(net_ensemble, val_loader, cfg)
            test_f1_score = F1_measure(net_ensemble, test_loader, cfg)
    ...
    print(f'Best validation stage: {best_stage},  best mcc@Val: {best_score:.4f}, final mcc@Test: {best_mcc_score:.4f}, final f1_score@Test: {best_f1_score:.4f}, final AUC_score@Test: {best_AUC_score:.4f}')
